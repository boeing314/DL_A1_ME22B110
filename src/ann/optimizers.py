...
class Momentum():
    def __init__(self,lr=0.01,momentum=0.9,weight_decay=0):
        ...
        self.v_b={}
    def update(self,layer):
        # We need to identify layer uniquely so that the velocity of different layers dont mix up
        layer_id=id(layer)
...
class RMSProp():
    ...
    def update(self,layer):
        ...
        if layer_id not in self.s_W:
            self.s_W[layer_id]=layer.W*0.0
            self.s_b[layer_id]=layer.b*0.0
        self.s_W[layer_id]=self.decay*self.s_W[layer_id]+(1-self.decay)*((layer.grad_W+self.weight_decay*layer.W)**2)
        self.s_b[layer_id]=self.decay*self.s_b[layer_id]+(1-self.decay)*(layer.grad_b**2)
        layer.W-=((self.lr*(layer.grad_W+self.weight_decay*layer.W))/(np.sqrt(self.s_W[layer_id])+self.eps))
        layer.b-=((self.lr*(layer.grad_b))/(np.sqrt(self.s_b[layer_id])+self.eps))